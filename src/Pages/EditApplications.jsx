...
const EditApplications = () => {
    ...
    const {jobTitle,companyName,firstName,lastName,phnNumber,graduationDate,linkedIn,coverletter,resume,skills,colleges,fieldOfStudy} = useLoaderData()
    const params = new URLSearchParams(location.search);
    const email = params.get('email');

    const onSubmit = (data) => {
        if(selectedOption) data.skills = selectedOption
        else data.skills = skills
        if(selectedClg) data.colleges = selectedClg
        else data.colleges = colleges
        if(selectedFs) data.fieldOfStudy = selectedFs
        else data.fieldOfStudy = fieldOfStudy
        data.date = new Date().toISOString().split('T')[0]
        console.log(data)
        fetch(`https://dashboard.render.com/web/srv-cq4i2no8fa8c73fpq3e0/update-application/${id}`,
            {
                method:"PATCH",
            ...
            reset()
            data.skills = null
            navigate(`/my-applications?email=${email}`);
        })
    }
                ...
                <form onSubmit={handleSubmit(onSubmit)} className='space-y-5'>
                    <div className='create-job-flex'>
                        ...
                        <div className='lg:w-1/2 w-full'>
                            ...
                            <input type="text" {...register("companyName")} defaultValue = {companyName}  className='create-job-input' />
                        </div>
                    </div>
                    ...
                    <div className='create-job-flex'>
                        <div className='lg:w-1/2 w-full'>
                            ...
                            <input type="text" defaultValue={firstName} placeholder="Alex" {...register("firstName")} className='create-job-input' />
                        </div>
                        <div className='lg:w-1/2 w-full'>
                            <label className='block mb-2 text-lg'>Last Name</label>
                            <input type="text" defaultValue={lastName} placeholder="John" {...register("lastName")} className='create-job-input' />
                        </div>
                    </div>
                    ...
                    <div className='flex flex-col lg:flex-row items-center justify-between gap-8'>
                        <div className='lg:w-1/2 w-full'>
                            ...
                            <input defaultValue={email} type="email" placeholder="Ex: alexjohn@gmail.com" {...register("email")} className='create-job-input' />
                        </div>
                        <div className='lg:w-1/2 w-full'>
                            <label className='block mb-2 text-lg'>Phone number</label>
                            <input defaultValue={phnNumber} type="text" placeholder="Ex: XXXXX XXXXX" {...register("phnNumber")} className='create-job-input' />
                        </div>
                    </div>
                    ...
                    <div className='create-job-flex'>
                        <div className='lg:w-1/2 w-full'>
                            ...
                            <input type="date" defaultValue={graduationDate} placeholder="Paste your company logo" {...register("graduationDate")} className='create-job-input' />
                        </div>
                        <div className='lg:w-1/2 w-full'>
                        <label className='block mb-2 text-lg'>Your LinkedIn profile</label>
                        <input type="text" defaultValue={linkedIn} placeholder="Paste your company logo" {...register("linkedIn")} className='create-job-input' />
                        </div>
                    </div>

                    <div className='w-full'>
                    <label className='block mb-2 text-lg'>Add your Cover letter</label>
                    <textarea defaultValue={coverletter} className='w-full pl-3 py-1.5 focus:outline-none' rows = {14} placeholder="Add your cover letter" {...register("coverletter")}/>
                    </div>
